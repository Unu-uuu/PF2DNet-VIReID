...
if dataset == 'sysu':
    ...
    for trial in range(10):
        ...
        gall_feat_pool, gall_feat_fc, gall_feat_fc_label = extract_gall_feat(trial_gall_loader)

        # fc feature
        #if test_mode == [1, 2]:
        if 1:
            distmat1 = np.matmul(query_feat_pool, np.transpose(gall_feat_pool))
            distmat2 = np.matmul(query_feat_fc, np.transpose(gall_feat_fc))
